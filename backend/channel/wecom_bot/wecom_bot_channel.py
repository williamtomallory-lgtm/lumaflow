...
class WecomBotChannel(ChatChannel):
    ...
    def _build_context(self, body: dict, is_group: bool, default_aeskey: str = ""):
        ...
        parse failure, etc.).
        """
        # A group-only Agent must never consume a private message even though
        # the WeCom transport itself supports both conversation types.
        if getattr(self, "bound_agent_id", ""):
            try:
                ...
                logger.warning("[WecomBot] Could not enforce agent chat type: %s", exc)
