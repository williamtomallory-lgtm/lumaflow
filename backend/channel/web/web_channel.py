...
def _lumaflow_primary_ui_url() -> str:
    """Return the trusted loopback UI used by an integrated LumaFlow install.

    CowAgent remains the API/runtime service, but its two browser entry points
    redirect to LumaFlow so administrators do not edit a second, divergent
    console. The opt-in environment variable preserves upstream standalone and
    Electron behaviour when unset.
    """
    raw = (os.environ.get("COW_LUMAFLOW_UI_URL") or "").strip()
...
class RootHandler:
    def GET(self):
        raise web.seeother(_lumaflow_primary_ui_url() or '/chat')
...
class ChatHandler:
    def GET(self):
        primary_ui = _lumaflow_primary_ui_url()
        if primary_ui:
            raise web.seeother(primary_ui)
...
class ChannelsHandler:
    ...
    CHANNEL_DEFS = OrderedDict([
        ...
        ("wecom_bot", {
            ...
            "fields": [
                ...
                {"key": "wecom_group_keywords_enabled", "label": {"zh": "启用关键词", "en": "Enable keywords"}, "type": "bool", "default": False},
                {"key": "wecom_group_keywords", "label": {"zh": "群聊关键词", "en": "Group keywords"}, "type": "text", "default": ""},
            ],
        }),
        ("qq", {
    ...
    def _channel_instances_view(cls) -> list:
        ...
        for inst in resolve_channel_instances(settings):
            ...
            if is_hant and isinstance(label_val, str):
                ...
                label_val = label_val.copy()
                label_val["zh-Hant"] = i18n.to_traditional(label_val.get("zh", ""))
            running = mgr.get_channel(inst.instance_id) if mgr else None
            login_status = getattr(running, "login_status", "") if running else ""
            out.append({
        ...
        try:
            ...
            if self._multi_agent_mode() and channel_name in MULTI_INSTANCE_READY and is_instance_op:
                if action == "save":
                    ...
                    return self._handle_instance_connect(
                        channel_name, instance_id, body.get("config", {}), body.get("agent_id", "")
                    )
    ...
    def _handle_instance_connect(self, channel_name: str, instance_id: str, updates: dict, agent_id: str = ""):
...
class WeixinQrHandler:
    ...
    # One independent login flow per channel instance.  The legacy key keeps
    # existing single-Agent desktop clients working unchanged.
    _qr_states = {}
    ...
    @staticmethod
    def _validate_identity(instance_id: str, agent_id: str):
        instance_id = (instance_id or "").strip()
        agent_id = (agent_id or "").strip()
        if not instance_id and not agent_id:
            return "", ""
        if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_-]{0,95}", instance_id):
            raise ValueError("invalid Weixin instance_id")
        if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_-]{0,63}", agent_id):
            raise ValueError("invalid Weixin agent_id")
        expected = f"weixin-{agent_id}"
    ...
    @classmethod
    def _state_key(cls, instance_id: str):
        return instance_id or "legacy"

    def GET(self):
        _require_auth()
        web.header('Content-Type', 'application/json; charset=utf-8')
        try:
            params = web.input(instance_id="", agent_id="")
            instance_id, agent_id = self._validate_identity(params.instance_id, params.agent_id)
            ...
            WeixinQrHandler._qr_states[self._state_key(instance_id)] = {
                "qrcode": qrcode,
    ...
    def POST(self):
        ...
        try:
            ...
            instance_id, agent_id = self._validate_identity(
                body.get("instance_id", ""), body.get("agent_id", "")
            )

            if action == "poll":
                return self._poll_status(instance_id, agent_id)
            elif action == "refresh":
                # GET reads query parameters, so refresh is handled by starting
                # a new flow through the same internal identity-aware helper.
                web.ctx.env["QUERY_STRING"] = urlencode({"instance_id": instance_id, "agent_id": agent_id})
                return self.GET()
            else:
                return json.dumps({"status": "error", "message": f"unknown action: {action}"})
        except Exception as e:
    ...
    def _poll_status(self, instance_id="", agent_id=""):
        state = WeixinQrHandler._qr_states.get(self._state_key(instance_id), {})
        ...
        if qr_status == "confirmed":
            ...
            if instance_id:
                from channel.channel_instances import upsert_instance
                inst = upsert_instance(
                    conf(), channel_type="weixin", instance_id=instance_id,
                    agent_id=agent_id,
                    credentials={"weixin_token": bot_token, "weixin_base_url": result_base_url},
                )

                def _start_instance():
                    try:
                        mgr = ChannelsHandler._channel_mgr()
                        if mgr is None:
                            raise RuntimeError("ChannelManager unavailable")
                        existing = mgr.get_channel(instance_id)
                        if existing is not None:
                            mgr.stop(instance_id)
                        mgr.add_channel(inst)
                    except Exception as exc:
                        logger.error("[WebChannel] Failed to start Weixin instance '%s': %s",
            ...
            WeixinQrHandler._qr_states.pop(self._state_key(instance_id), None)
            logger.info(f"[WebChannel] WeChat QR login confirmed: bot_id={bot_id}, instance={instance_id or 'legacy'}")
...
def _session_model_catalog() -> List[dict]:
    ...
    the environment rather than in config.json.  The LumaFlow demo rail is the
    one deliberate exception: it is static catalog data, always visible, and
    never probes or downloads a model.  Its selectable local entry routes via
    the existing ``custom`` OpenAI-compatible bot; its remote entry is exposed
    only through ``model_options`` and is disabled by the desktop selector.
    """
    local_config = conf()
    ...
    # LumaFlow picker presets are intentionally appended after configured
    # vendors so the existing provider ordering remains stable.  ``models``
    # stays a plain string list for older consumers (AgentsPage and legacy
    # clients); ``model_options`` carries the richer display/availability
    # metadata used by the session selector.  This is pure data assembly — no
    # Ollama probe, Hugging Face request, or download path is involved.
    lumaflow_entries = model_catalog.get_lumaflow_model_presets()
    selectable_entries = [
        entry for entry in lumaflow_entries
        if entry.get("selectable", True) and not entry.get("disabled", False)
    ]
    selectable_models = [entry["name"] for entry in selectable_entries]
    if (
        active_provider == model_catalog.LUMAFLOW_ROUTE_PROVIDER
        and active_model
        and active_model != model_catalog.LUMAFLOW_REMOTE_MODEL_ID
        and active_model not in selectable_models
    ):
        # Preserve a user-pinned custom model without turning the remote
        # placeholder into an executable option.
        selectable_models.insert(0, active_model)

    model_options = []
    for entry in lumaflow_entries:
        option = {
            "value": entry["name"],
        ...
        # Avoid leaking absent optional values into the wire payload while
        # keeping the explicit false flags that make the placeholder safe.
        model_options.append({key: value for key, value in option.items() if value is not None})

    catalog.append({
        # Keep the id executable by the existing session/runtime route.  The
        # display label identifies this as the LumaFlow preset group, while
        # custom remains the only bot type needed for the local endpoint.
        "id": model_catalog.LUMAFLOW_ROUTE_PROVIDER,
        "preset_id": model_catalog.LUMAFLOW_PROVIDER_ID,
        "label": {"zh": "LumaFlow", "en": "LumaFlow"},
        "route_provider": model_catalog.LUMAFLOW_ROUTE_PROVIDER,
        "backend": "ollama",
        "api_base": model_catalog.LUMAFLOW_OLLAMA_API_BASE,
        "downloadable": False,
