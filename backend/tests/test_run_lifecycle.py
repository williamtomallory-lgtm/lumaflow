...
class RunLifecycleTest(unittest.TestCase):
    ...
    def test_execution_decision_is_persisted_with_the_run(self):
        decision = {
            "mode": "hybrid",
            "intent": "lookup_then_business_work",
            "confidence": 0.94,
        }
        run_id, token, store = self.bridge._begin_run(
            "s1", "sales", _context(execution_decision=decision)
        )

        self.assertEqual(self.store.get_run(run_id)["extras"]["execution"], decision)
        self.bridge._end_run(store, run_id, token, "done")
