...
export interface SessionModelProvider {
  ...
  /** Runtime provider used when a display/catalog id is only a preset alias. */
  route_provider?: string
  /** Stable preset id for product-owned catalog groups, when present. */
  preset_id?: string
  models: string[]
  /** Optional annotated selector entries; `models` remains string[] for
   * legacy consumers and Agent profile editing. */
  model_options?: ModelEntry[]
  /** Optional metadata for a product-owned model rail. */
  backend?: string
  api_base?: string
...
export interface ModelOption {
  value: string
  hint?: string
  /** Human-facing label; omitted entries fall back to `value`. */
  label?: string
  /** Where the model is served from, if known. */
  source?: 'local' | 'remote'
  /** Serving adapter, if known. */
  backend?: 'ollama' | 'openai-compatible' | 'hugging-face' | string
  /** Availability status used by model selectors. */
  status?: 'available' | 'not-installed' | 'disabled' | 'remote' | string
  /** Explicit opt-in for selector activation; defaults to true. */
  selectable?: boolean
  /** Redundant but convenient UI flag for disabled placeholders. */
  disabled?: boolean
  /** Whether selecting this entry may trigger an install/download. */
  downloadable?: boolean
}
