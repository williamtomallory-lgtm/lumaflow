...
import { normEntry } from '../pages/settings/modelsHelpers'
import type { ModelEntry, SessionModelProvider } from '../types'
...
// Rich catalog entries are authoritative for their metadata, but the legacy
// string list may contain an active custom model that is not part of the
// static preset list. Keep those extra strings visible for old and new APIs.
const selectorEntries = (provider: SessionModelProvider): ModelEntry[] => {
  const options = provider.model_options || []
  if (!options.length) return provider.models || []
  const known = new Set(options.map((entry) => (typeof entry === 'string' ? entry : entry.value)))
  return [...options, ...(provider.models || []).filter((model) => !known.has(model))]
}
    ...
    >
      ...
      {providers.map((p, idx) => (
        <React.Fragment key={p.id}>
          ...
          {selectorEntries(p).map((rawModel) => {
            // Legacy providers still send string model ids. Product-owned
            // catalogs can send an annotated model_options entry so a remote
            // placeholder remains visible without becoming executable.
            const option = normEntry(rawModel)
            const modelId = option.value
            const routeProvider = p.route_provider || p.id
            const selectable =
              option.selectable !== false &&
              option.disabled !== true &&
              option.status !== 'not-installed' &&
              option.status !== 'disabled' &&
              option.status !== 'remote'
            const active = modelId === activeModel && routeProvider === activeProvider
            return (
              <button
                key={`${p.id}:${modelId}`}
                type="button"
                onClick={() => {
                  ...
                  select(active && pinned ? null : routeProvider, active && pinned ? null : modelId)
                }}
                disabled={!selectable}
                ...
                className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-left transition-colors ${
                  !selectable
                    ? 'text-content-tertiary opacity-60 cursor-not-allowed'
                    : active
                      ? 'bg-accent-soft text-accent cursor-pointer'
                      : 'hover:bg-surface-2 text-content cursor-pointer'
                }`}
              >
                <Cpu size={13} className="shrink-0" />
                <span className="flex-1 min-w-0 text-[13px] truncate">{option.label || modelId}</span>
                {!selectable && (
