...
describe("local knowledge archive", () => {
  ...
  it("deletes both metadata and original bytes", async () => {
    const saved = await saveUploadedKnowledge({ originalName: `delete-${randomUUID()}.txt`, mimeType: "text/plain", bytes: Buffer.from("temporary verification file"), parse: { status: "parsed", text: "temporary verification file", characters: 27 } });
