        ...
        <div className={styles.composerArea}>
          <div className={styles.composer}>
            ...
            {experience === "work" && <div className={styles.roleRow}>
              <label><Sparkles size={14} /><select aria-label="选择后端 Agent" value={selectedWorkAgent?.id || ""} disabled={busy || agentLoading || !enabledAgents.length} onChange={(event) => changeAgent(event.target.value)}>{!enabledAgents.length && <option value="">{agentLoading ? "正在读取 CowAgent…" : "没有可用 Agent"}</option>}{enabledAgents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name} · {agent.agentType === "wecom_group" ? "群聊" : agent.agentType === "weixin_personal" ? "个人微信" : "通用"}</option>)}</select></label>
              {(selectedWorkAgent?.agentType === "weixin_personal" || rolePresetId === "wechat-service") && <button type="button" disabled={busy} onClick={() => setWechatDialogOpen(true)}><Upload size={14} /> 导入微信记录</button>}
            </div>}
          ...
          {contextOpen && <section className={styles.contextPanel} data-testid="knowledge-picker" aria-label="参考资料与客户">
            <div className={styles.panelHeading}><h3>本轮参考资料</h3><span>{selectedDocumentIds.length} / {Math.min(50, knowledgeDocuments.filter((document) => document.selectable).length)}</span>{onOpenKnowledge && <button type="button" onClick={onOpenKnowledge}><Upload size={14} /> 去知识库上传</button>}</div>
